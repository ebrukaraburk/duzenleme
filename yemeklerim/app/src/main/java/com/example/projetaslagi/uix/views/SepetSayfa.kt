...
fun SepetSayfa(
    ...
    val coroutineScope = rememberCoroutineScope() // CoroutineScope oluşturma
    ...
    Scaffold(
                ...
                Button(
                    onClick = {
                        coroutineScope.launch {
                            // Siparişi onayla işlemi
                            //sepetViewModel.sepetiTemizle(kullaniciAdi) // Sepeti temizleme
                            navController.navigate("anaSayfa") // Ana sayfaya yönlendirme
                            snackbarHostState.showSnackbar("Sepetiniz onaylandı") // Mesaj gösterme
                        }
                    },
        ...
        Column(
            ...
            if (sepetListesi.isEmpty()) {
                Text(text = "Sepetiniz boş.", style = MaterialTheme.typography.bodyLarge)
            } else {
                LazyColumn {
                    items(sepetListesi) { yemek ->
                        SepetItem(yemek = yemek, sepetViewModel = sepetViewModel, kullaniciAdi = kullaniciAdi)
                    }
                }
            }
        }
...
fun SepetItem(yemek: Sepet, sepetViewModel: SepetViewModel, kullaniciAdi: String) {
    Card(
        ...
        colors = CardDefaults.cardColors(containerColor = colorResource(id = R.color.white)) // Change to white
    ) {
        Box(
            modifier = Modifier
                .fillMaxWidth()
            ...
            IconButton(
                onClick = { sepetViewModel.sil(yemek.sepet_yemek_id, kullaniciAdi) },
